...
export async function indexVaultFiles(
  ...
  const notice = new Notice(`Vault Chat Assistant: indexing 0/${files.length} files...`, 0);
  indexStore.clear();
  ...
  try {
    for (let index = 0; index < files.length; index += 1) {
      ...
      if (done === files.length || done % 10 === 0) {
        notice.setMessage(`Vault Chat Assistant: indexing ${done}/${files.length} files...`);
      }
    }

    const coverage = indexStore.getCoverage();
    notice.setMessage(`Vault Chat Assistant: indexed ${coverage.indexedFiles}/${coverage.totalFiles} files.`);
